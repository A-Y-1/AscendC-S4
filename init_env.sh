...
if [ -e "Ascend-cann-toolkit_8.0.0.alpha003_linux-aarch64.run" ]; then
    file_size=$(ls -l "Ascend-cann-toolkit_8.0.0.alpha003_linux-aarch64.run" | awk '{print $5}')
    full_size=2408159899
    if [ $file_size -lt $full_size ]; then
        echo "CANN包大小不正确，需要重新下载"
        wget https://ascend-repo.obs.cn-east-2.myhuaweicloud.com/Milan-ASL/Milan-ASL%20V100R001C20SPC703/Ascend-cann-toolkit_8.0.0.alpha003_linux-aarch64.run -O Ascend-cann-toolkit_8.0.0.alpha003_linux-aarch64.run
        chmod +x Ascend-cann-toolkit_8.0.0.alpha003_linux-aarch64.run >/dev/null 2>&1
    fi
...
if [ ! -d "/home/ma-user/Ascend/ascend-toolkit/8.0.0.alpha003" ]; then
    

    ./Ascend-cann-toolkit_8.0.0.alpha003_linux-aarch64.run --install --force --quiet >install.log 2>&1 &
