...
def _build_args(y_in__, num_rows, num_columns, batch_shape, dtype):
    __inputs__ = []
    for arg in [y_in__]:
        if arg != None:
...
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_LIST_INT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def eye(y_in__, num_rows, num_columns=0, batch_shape=[], dtype=0, kernel_name="eye", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, num_rows, num_columns, batch_shape, dtype)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[y_in__], origin_outputs = [],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required'], init_value_list = [],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(y_in__, num_rows, num_columns=0, batch_shape=[], dtype=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, num_rows, num_columns, batch_shape, dtype)
    result = check_op_cap("op_select_format", "Eye", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(y_in__, num_rows, num_columns=0, batch_shape=[], dtype=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, num_rows, num_columns, batch_shape, dtype)
