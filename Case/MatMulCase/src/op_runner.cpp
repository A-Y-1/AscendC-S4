...
bool OpRunner::RunOp()
{
    ...
    // TODO 4：修改GetWorkSpace算子接口调用
    auto ret = aclnnMatMulGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2], outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    // TODO 5：修改算子接口调用
    ret = aclnnMatMul(workspace, workspaceSize, handle, stream);
