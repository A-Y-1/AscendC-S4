...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    size_t zBufferSize = zTotalBlock * BLOCK_SIZE * 2;              //暂存实部虚部4部分矩阵乘结果的空间，每一部分是原来的z的1/2
    size_t biasBufferSize = hasBias?(zTotalBlock*BLOCK_SIZE):0;     //分离bias实部虚部的空间
    tiling.SaveToBuffer(context->GetRawTilingData()->GetData(), context->GetRawTilingData()->GetCapacity());
    context->GetRawTilingData()->SetDataSize(tiling.GetDataSize());
    size_t userWorkspaceSize = xBufferSize + yBufferSize + zBufferSize + biasBufferSize;
